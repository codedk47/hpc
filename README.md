...
所以不要请别抱怨目前服务端对全局连接表操作功太少，这里多线程处理不当可能让这个 socket 失去原来的逻辑事件控制机制
应为你永远无法控制当吧这个 socket 丢进 IOCP 队列后它下次会出现在那个线程里，只有取消这个 IOCP 投递方法
那么这个服务端其实就是一个很简单的服务端，并没有太多花哨操作API，所有代码加起来可能不足4千行
其中包括 IOCP基本网络库封装(OpsenSSL兼容)、hash_node、sync_list、嵌入PHP、5个PHP操作类扩展
除此之外并未含有其他任何库，所以它很精简，也只能运行在 Windows 平台上
</pre>
